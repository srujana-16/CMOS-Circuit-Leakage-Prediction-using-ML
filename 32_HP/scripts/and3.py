...
# Load the PVT data
pvt_data = pd.read_csv('../../data/and3_data.csv')
...
def run_leakage_simulation(pvt_row):
    ...
    .END
    """
    with open('../netlists/and3_leakage.sp', 'w') as f:
        f.write(netlist)
    
    # subprocess.run(['ngspice', '-b', 'nand2_leakage.sp'])
    command = "ngspice -b ../netlists/and3_leakage.sp > ../netlists/and3_leakage.txt"
    ...
    with open('../netlists/and3_leakage.txt', 'r') as f:
...
results.to_csv('../outputs/simulation_results_and.csv', index=False)