...
# Load the PVT data
pvt_data = pd.read_csv('../data/c499_data_test.csv')
...
def run_leakage_simulation(pvt_row):
    ...
    * let P_Vdd = abs(I(Vdd) * pvdd)
    ...
    .end
    """
    with open('./c499_leakage.sp', 'w') as f:
        f.write(netlist)
    
    # subprocess.run(['ngspice', '-b', 'nand2_leakage.sp'])
    command = "ngspice -b ./c499_leakage.sp > ./c499_leakage.txt"
    ...
    with open('./c499_leakage.txt', 'r') as f:
...
results.to_csv('./simulation_results_c499.csv', index=False)